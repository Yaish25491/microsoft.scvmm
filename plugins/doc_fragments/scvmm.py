  ...
  auth:
    description:
      ...
      - V(kerberos) requires the C(pypsrp[kerberos]) extra and the system
        ...
        dependencies and is the default for Execution Environments.
      - When the C(pypsrp[kerberos]) extra is not installed, V(negotiate)
        cannot negotiate Kerberos and falls back to NTLM, which is weaker.
        Install the Kerberos extra (or use V(kerberos) explicitly) if NTLM
