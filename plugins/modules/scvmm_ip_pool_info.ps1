...
try {
    ...
    $pools = Get-SCStaticIPAddressPool @cmdletArgs

    if ($pools) {
        # Ensure we always process an array even if a single item is returned
        $poolsArray = @($pools)
        foreach ($pool in $poolsArray) {
            $module.Result.ip_pools += Get-SCVMMIPPoolInfo -IPPool $pool
        }
    }
}
catch {
    $module.FailJson("Failed to gather IP address pool info: $($_.Exception.Message)", $_)
}
