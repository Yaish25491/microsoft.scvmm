...
function Get-PoolResult {
    ...
    $result['host_groups'] = @($Pool.HostGroups | ForEach-Object { $_.Name })
    return $result
}
...
if ($module.Params.state -eq 'present') {
    if (-not $pool) {
        ...
        # Host group membership is fixed at creation time; warn (like the MAC
        # range) instead of attempting to change it.
        if ($null -ne $module.Params.host_groups) {
            $currentGroups = @($pool.HostGroups | ForEach-Object { $_.Name }) | Sort-Object
            $desiredGroups = @($module.Params.host_groups) | Sort-Object
            $groupsDiffer = $false
            if ($currentGroups.Count -ne $desiredGroups.Count) {
                $groupsDiffer = $true
            }
            elseif ($currentGroups.Count -gt 0) {
                $diff = Compare-Object -ReferenceObject $currentGroups -DifferenceObject $desiredGroups -ErrorAction SilentlyContinue
                $groupsDiffer = [bool]$diff
            }
            if ($groupsDiffer) {
                $cur = $currentGroups -join ', '
                $req = $desiredGroups -join ', '
