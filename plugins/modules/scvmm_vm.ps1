...
$spec = @{
    options = @{
        ...
        cpu_count = @{ type = 'int' }
        memory_mb = @{ type = 'int' }
        dynamic_memory = @{ type = 'bool'; default = $false }
...
if ($state -eq 'absent') {
    ...
    if (-not $vm) {
        ...
        if ($dynamicMemory) {
            $createParams.DynamicMemoryEnabled = $true
        }
        if ($generation) {
        ...
        if ($cpuCount -and $vm.CPUCount -ne $cpuCount) {
            $updateParams.CPUCount = $cpuCount
            $needsUpdate = $true
        }

        if ($memoryMb -and $vm.Memory -ne $memoryMb) {
            $updateParams.MemoryMB = $memoryMb
            $needsUpdate = $true
        }
        ...
        if (-not $module.CheckMode) {
            ...
            if ($cpuCount) {
                $afterState.cpu_count = $cpuCount
            }
            if ($memoryMb) {
                $afterState.memory_mb = $memoryMb
            }
            if ($description) {
                $afterState.description = $description
            }
