...
options:
  ...
  vm_host:
    description:
      ...
      - If not specified, SCVMM will automatically select a host.
    type: str
    required: false
  cloud:
    description:
      - Cloud to deploy the cloned VM into.
      - Mutually exclusive with C(vm_host) in some configurations.
    type: str
    required: false
  vmm_server:
    description:
...
source_vm:
  ...
  sample: WebServer01
state:
  description: State of the cloned virtual machine.
  returned: always
  type: str
  sample: present
vm:
